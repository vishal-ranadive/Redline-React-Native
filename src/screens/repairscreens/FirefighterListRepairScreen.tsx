...
export default function FirefighterListRepairScreen() {
  ...
  const fetchAllRepairs = useCallback(async (leadId: number, showLoading: boolean = true, name?: string) => {
    try {
      if (showLoading) {
        setLoading(true);
      }
      ...
      // Fetch rosters with repairs included
      const rostersResponse = await repairApi.getRepairRosters(leadId, page, numberOfItemsPerPage, name);
  ...
  useEffect(() => {
    if (isFirstMount.current) {
      fetchAllRepairs(effectiveLeadId, true, debouncedSearchQuery);
      isFirstMount.current = false;
    }
  }, [effectiveLeadId, fetchAllRepairs, debouncedSearchQuery]);
  ...
  }, [page, numberOfItemsPerPage, effectiveLeadId, fetchAllRepairs, debouncedSearchQuery]);

  // Refresh on focus (but don't show loading spinner)
  useFocusEffect(
    useCallback(() => {
      if (!isFirstMount.current) {
        fetchAllRepairs(effectiveLeadId, false, debouncedSearchQuery);
      }
    }, [effectiveLeadId, fetchAllRepairs, debouncedSearchQuery]),
  );
  ...
  const handleRefresh = useCallback(async () => {
    setRefreshing(true);
    await fetchAllRepairs(effectiveLeadId, false, debouncedSearchQuery);
  }, [effectiveLeadId, fetchAllRepairs, debouncedSearchQuery]);
  ...
  /**
   * Render roster group with repair gears
   */
  const renderRosterGroup = (item: RosterGroup) => {
    ...
    return (
      <View style={[styles.cardWrapper, isMobile && styles.cardWrapperMobile]}>
        <TouchableOpacity
          activeOpacity={0.8}
          onPress={() => handleViewRepairs(item)}
        >
      ...
      {loading && !refreshing ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text variant="bodyMedium" style={{ color: colors.outline, textAlign: 'center', marginTop: 8 }}>
            {error ?? (debouncedSearchQuery ? 'No firefighters found matching your search' : 'No rosters with repair data available')}
          </Text>
        </View>
      ) : (
        <ScrollView
